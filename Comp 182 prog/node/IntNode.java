...
Program Assignment #5  
   ...
   //get depth of closest leaf   
      ...
      private static int closeLeaf(IntNode T){ 
         int result=0,leftCount,rightCount;    
         if(T==null) 
            result= 0; 
         else if(T.getLeft()==null && T.getRight() == null)  
            result= 1; //when its a leaf make result 1
         else{ 
            leftCount = closeLeaf(T.getLeft()); 
            rightCount = closeLeaf(T.getRight()); 
            if(leftCount==0)  //take into account there is no left subtree
               leftCount= rightCount+1; 
            if(rightCount==0)  //take into account no right subtree
               rightCount= leftCount+1; 
            if(leftCount>rightCount) 
               result=rightCount+1; //add one to take care of root
            else
               result=leftCount+1; //add one to take care of root
   ...
   //get number of leaves in BST   
      ...
      private static IntNode delete(IntNode T, int num){    //T(n)=O(logn)   
         IntNode Ptr=T; 
         IntNode Pred=T;   
